;;; PolyDos utility: SCRAPE2 Copy 256-byte/sector disks
;;; to SDcard.
;;;
...
;;; routines to do the disk access - therefore, it must be run
;;; from a FDD-booted PolyDos system.
;;; SCRAPE5.asm copies 512-byte/sector disks. It cannot
;;; use PolyDos SCALs but must contain its own disk access code -
;;; therefore it can be run from a FDD-booted PolyDos system
;;; or a SD-card-booted PolyDos system, or "barefoot" on the
;;; hardware (loaded from tape etc.)
;;; SCRAPE2.asm copies 256-byte/sector disks but contains its
;;; own disk access code - therefore it can be run from a
;;; FDD-booted PolyDos system or a SD-card-booted PolyDos system,
;;; or "barefoot" on the hardware (loaded from tape etc.)
;;;
;;; All version accesses nascom_sdcard directly for storing
;;; scraped data, but initialise it first. This means that the
;;; drives may be reassigned and so it may be necessary to
;;; reboot after exiting this program.
;;;
...
CNVSAD:	PUSH	HL		;Save
	...
	LD	BC,18*2		;(Double density size)
