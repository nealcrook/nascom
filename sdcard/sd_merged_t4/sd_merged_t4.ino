...
//    pinMode(PIN_H2T, INPUT);
    ...
    NASSERIAL.print("\x1f");
    NASSERIAL.print("X0\x1f"); // put NAS-SYS into echo mode
    NASSERIAL.print(" Type some characters\x1f");
    for (int i=0; i<10; i++) {
        while (! NASSERIAL.available()) {
        }
        NASSERIAL.print(" Read ");
        NASSERIAL.print(NASSERIAL.read());
        NASSERIAL.print("\x1f");
    }
    NASSERIAL.print("Thanks, bye.\x1f");
#endif
...
void pr_msg(const char *msg, char flags) {
    ...
    if (flags & F_MSG_CR) {
        NASSERIAL.print("\x1f"); // TODO maybe do this explicitly. What does NASCOM need? CR LF or both? this does \r\n
    }
...
void cmd_cass(void) {
    ...
    case ('I'<<8 | 'N'):      // INFO - version and status
        ...
        NASSERIAL.print(pbuf);

        NASSERIAL.print(F("\x1fRead  name: "));
        NASSERIAL.print(cas_rd_name);
        NASSERIAL.print(F("\x1fWrite name: "));
        NASSERIAL.print(cas_wr_name);
        NASSERIAL.print(F("\x1fVdisk name: "));
        NASSERIAL.print(cas_vdisk_name);
        NASSERIAL.print(F("\x1f"));
        break;
...
void cmd_disk() {
    ...
    DEBSERIAL.print(F("Command "));
    DEBSERIAL.println(cmd_data,HEX);
...
int prestore(char pid) {
    ...
    if (csum == 0) {
        DEBSERIAL.println(F("using profile record"));
        if (pid == 7) {
            pid = EEPROM.read(3);
        }
...
//    pinMode(PIN_H2T, INPUT);
    ...
    DEBSERIAL.println(F("cmd_stop - wait for reset"));
    while (1) {
    }
}
...
void cmd_ts_seek(char fid) {
    ...
    if (handles[fid]) {
        //    DEBSERIAL.print("Seek to track ");
        //    DEBSERIAL.print(track,HEX);
        //    DEBSERIAL.print(" sector" );
        //    DEBSERIAL.println(sector,HEX);
        ...
        DEBSERIAL.print(F("Seek to track but no disk"));
    }
    put_value(status, INPUT);
}
...
void n_wr(char fid, long count) {
    ...
    //  DEBSERIAL.print("Write byte count ");
    //  DEBSERIAL.println(count,HEX);
...
void n_rd(char fid, long count) {
    ...
    //  DEBSERIAL.print("Read for fid ");
    //  DEBSERIAL.print(fid,HEX);
    //  DEBSERIAL.print(" and byte count ");
    //  DEBSERIAL.println(count,HEX);
