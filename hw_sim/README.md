...
Each line of characters is made up (vertically) of a sequence of raster scan-lines on the
...
video). In both cases, there is no additional vertical spacing between the lines of
characters.

This difference between NASCOM 1 and NASCOM 2 explains why you can't see the
"feet" on the stick figure character on the NASCOM 2.
...
2). Each time, the RAM returns the same value (the 7-bit ASCII code of the
character to be displayed) and a 4-bit counter increments.  Together, the ASCII
code and the 4-bit count are used to address the character generator, to get the
8-bit value that represents the 8 dots for this 1/64th part of this scan-line.
