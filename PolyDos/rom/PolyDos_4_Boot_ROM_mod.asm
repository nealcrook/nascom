...
;	5.25" single sided 80 track floppy disk
;	drives (Micropolis 1015 or equivalent)
;
;----------------------------------------------
...
CRDSEC:	EQU	088H	;Read sector
CWRSEC:	EQU	0A8H	;Write sector
...
; Start motors and test that a disk is present

MOTON:	LD	A,(DRVCOD)	;Get drive code
	OUT	(DRPORT),A	;Address it
	IN	A,(STSREG)	;Motors running and
	AND	80H		;head loaded?
	RET	Z		;Yes => skip
	IN	A,(TRKREG)	;Do a dummy seek to get
	OUT	(DATREG),A	;head loaded
	LD	A,CSEEK
	CALL	C1797
	PUSH	HL		;Save registers
	PUSH	BC
	LD	HL,2000		;Set timeout count
MO1:	IN	A,(STSREG)	;Read status
	AND	80H		;Is NOT READY low?
	JR	Z,MO2		;Yes => skip
	LD	B,1		;Delay for 1 ms
	CALL	DELAY
	DEC	HL		;Decrement count
	LD	A,H		;Timeout?
	OR	L
	JR	NZ,MO1		;No => retry
	LD	A,27H		;Load error code
MO2:	POP	BC		;Restore registers
	POP	HL
	OR	A		;Status to Z flag
	RET
...
DRSEL:	LD	A,MAXDRV	;Too big?
	...
	LD	C,A		;Put in C
	CALL	MOTON		;Start motors
	JR	NZ,DS1		;No disk => skip
	LD	A,B		;Already selected?
	SUB	C
	JR	Z,DS1		;Yes => skip
	LD	B,100		;Delay in case head
	CALL	DELAY		;loading
	PUSH	HL		;Save registers
	PUSH	DE
