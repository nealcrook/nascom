...
;;; 4 physical drives. 0-3 treat them as double density, 4-7 treat
;;; them as single density
MAXDRV:	EQU	7
FFLP:	EQU	0045H
...
PDC3:	LD	(HL),0
	...
	LDIR			;Copy to SCTB
	LD	HL,SCTBS	;Activate new SCAL table
	LD	(STAB),HL
	...
	SCAL	ZSSCV
	DB	ZNNIM
	SCAL	ZNNIM		;Activate new input table
	LD	HL,POUT		;Make printer user output
	LD	(UOUTA),HL	;device
...
LK2:	LD	HL,20		;Point to next directory
	ADD	HL,DE		;FCB
	EX	DE,HL		;Put in DE
...
LK3:	PUSH	HL		;Save lookup FCB addr
	PUSH	DE		;Save directory FCB addr
...
LK8:	POP	DE		;Restore dir FCB addr
	POP	HL		;Restore lookup FCB addr
...
ENT1:	LD	DE,(NXTFCB)	;Is directory full?
	...
	ADD	HL,DE
	LD	(NXTSEC),HL
	SCAL	ZWDIR		;Write directory to disk
ENT2:	POP	HL		;Restore
	POP	BC
...
GETOV:	PUSH	AF		;Save all
	...
	LD	B,4		;Blank fill rest of name
GOV1:	LD	(HL),' '
	INC	HL
	DJNZ	GOV1
...
CKER:	OR	A		;Error?
	...
	LD	(ERRFLG),A
DBREAK:	SCAL	ZCFMA		;Abort command file mode
...
SSCV:	EX	(SP),HL		;Get routine number
	...
	LD	D,(HL)		;Read old
	LD	(HL),B		;Save new
	EX	DE,HL		;Put old vector into HL
	RET
...
PO5:	LD	A,(PCPL)	;Are we at right margin?
	CP	(HL)
	JR	NZ,PO6		;No => skip
	PUSH	BC
	PUSH	HL
...
BLINK:	LD	A,(CFFLG)	;Command file mode?
	OR	A
	JR	NZ,BL3		;Yes => skip
BL1:	LD	HL,(CURSOR)	;Get character at cursor
	LD	A,(HL)
...
DSIZE:	LD	A,MAXDRV	;Too big?
	...
	RET	C		;Yes => return
	XOR	A		;No error
	BIT	2,C		;Double density?
	LD	HL,35*18*2	;(Double density size)
	RET	Z		;Yes => return
	LD	HL,35*10*2	;Single density size
	XOR	A		;No error
	RET
...
RWS1:	POP	AF		;Restore R/W flag
	...
	LD	A,(DRVCOD)	;Double density?
	BIT	4,A
	LD	C,18*2		;(Double density size)
	JR	Z,RWS2		;Yes => skip
	LD	C,10*2		;Single density size
...
CNVSAD:	PUSH	HL		;Save
	...
	BIT	4,A		;Double density?
	LD	BC,18*2		;(Double density size)
	JR	Z,CSA1		;Yes => skip
	LD	BC,10*2		;Single density size
...
RDWR:	PUSH	BC		;Save
	...
	LD	A,(DRVCOD)	;Get drive code
	BIT	4,A		;Double density?
	LD	D,18		;(18 sectors/track)
	JR	Z,RW0		;Yes => skip
	LD	D,10		;10 sectors/track
...
RWR7:	POP	BC		;Restore
	...
	DEFB 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF
	DEFB 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF
	DEFB 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF
